function [seis,bc_top,bc_bottom,bc_left,bc_right,bc_p_nt,bc_p_nt_1]...
    =a2d_bmod_abc28_snapshot(v,refl,nbc,dx,nt,dt,s,sx,sz,gx,gz,isFS)
...
for it=1:nt
    ...
    if mod(it,100)==1
        figure(2);subplot(234);colormap(gray);imagesc(x,z,p(nbc+1:nbc+nz,nbc+1:nbc+nx));
        figure_title=['Background wave field P, t=',num2str((it-1)*dt),' s'];
        title(figure_title);ylabel('Z (m)');xlabel('X (M)');caxis([-0.25 0.25]); 
        figure(2);subplot(235);colormap(gray);imagesc(x,z,q(nbc+1:nbc+nz,nbc+1:nbc+nx));
        figure_title=['Purtubation wave field Q, t=',num2str((it-1)*dt),' s'];
        title(figure_title);ylabel('Z (m)');xlabel('X (M)');caxis([-1 1]); 
        figure(2); subplot(236);imagesc(g,t,seis);
