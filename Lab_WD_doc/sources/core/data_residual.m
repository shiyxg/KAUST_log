function [ data_res ] = data_residual(seis,csg_true,method)
...
%         : 3 f -k domain  : AWI
%         : 4 f -k domain  : w-AWI, divide gather into sub-section with tw (tw: trace window);with overlaping%         
...
%             Rr(:,:,k) = (F_sub_seis(:,:,k)./(abs(F_sub_seis(:,:,k))+0.001*max(max(abs(F_sub_seis(:,:,k)))))).*(abs(F_sub_seis(:,:,k))-abs(F_sub_csg_true(:,:,k)));
